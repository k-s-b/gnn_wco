...
class BaseGNNStack(nn.Module):
    def __init__(self,layer_dims, gnn_type):
        '''
        Create a model that stacks multi-layer GNN.
        The embedding obtainable are concatenated as output. 
        '''
        ...
        for i in range(len(layer_dims) -1):
            gnn_module = GNN(layer_dims[i], layer_dims[i+1], gnn_type=gnn_type)
            self.gnns.append(gnn_module)
...
class BaseGNN(LightningModule):
    def __init__(self, input_dim, hidden_dim, numLayers, useXGB=True):
        ...
        # output
        self.outLayer = nn.Linear(int(hidden_dim * (self.numLayers + 1)), 1)
    ...
    def forward(self, x,adjs):
        ...
        # logits
        logits = [self.outLayer(embeddings)]
